    ...
    return (
        <>
            ...
            <div className="max-w-7xl mx-auto px-4 py-8">
                <h1 className="text-2xl font-bold text-white mb-6">📜 হাদীস সংকলন</h1>
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                    {hadithBooks.map((book) => (
                        <Link
                            key={book.slug}
                            to={`/hadith/${book.slug}`}
                            className="bg-[#0d1428] rounded-xl border border-blue-800/40 p-6 hover:border-[#c9a84c]/50 gold-glow transition-all group"
                        >
